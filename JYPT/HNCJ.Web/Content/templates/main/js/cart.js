...
function cartAdd(obj, webpath, linktype, linkurl){
	...
	var goodsPrice = parseFloat($("#GoodsPrice").val());
	...
	//如果是立即购买
	if(linktype == 1){
	    var jsondata = '[{ goods_id:' + goodsId + ', goods_count:' + selectNum + ',goods_price:' + goodsPrice + '}]'; //结合商品参数
		$.ajax({
			type: "post",
			url: webpath + "OrderInfo/AddCart",
			...
			success: function(data, textStatus) {
				if (data.status == "ok") {
					...
					var HintHtml = '<div id="cartInfoHint" class="msg-tips cart-info">'
						...
						+ '<p><a href="/UserLogin/Login">' + data.msg + '</a></p>'
						+ '<i class="close" title="关闭" onclick="hintRemove(\'#cartInfoHint\');"><span>关闭</span></i>'
						+ '</div>'
						+ '</div>'
					$(obj).after(HintHtml); //添加节点
...
function updateCart(obj, webpath, num){
	...
	var articleId = $(obj).parents("tr").find("input[name='hideArticleId']").val(); //购物车ID
	...
	$.ajax({
		type: "post",
		url: webpath + "orderInfo/updateCart",
		data: {
			cart_id : articleId,
			goods_id : goodsId,
...
function deleteCart(webpath, obj){
	...
	if(arglength == 1){
		...
		datastr = { cart_id: articleId, goods_id: goodsId }
	}
	$.ajax({
		type: "post",
		url: webpath + "OrderInfo/DeleteCart",
		data: datastr,
		dataType: "json",
		beforeSend: function(XMLHttpRequest) {
			//发送前动作
...
function cartAmountTotal(){
	...
	$(".checkall:checked").each(function(i) {
		...
		jsondata += '{cart_id:"'+articleId+'", goods_id:"'+goodsId+'", quantity:"'+goodsQuantity+'"}';
...
function cartSubmitID()
{
    ...
    window.location.href = "/orderInfo/shopping?ids=" + str;
}
//进入结算中心
function formSubmit(obj, webpath, linkurl){
	...
	$.ajax({
		type: "post",
		url: webpath + "orderInfo/Shopping",
		data: { list: jsondata },
		dataType: "json",
		beforeSend: function(XMLHttpRequest) {
			//发送前动作
